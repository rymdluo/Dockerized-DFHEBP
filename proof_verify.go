...
func (proof *Proof) Verify(s *Statement) bool {

	var anonsupport AnonSupport
	var protsupport ProtocolSupport
	var sigmasupport SigmaSupport

	gparams := NewGeneratorParams(128) // these can be pregenerated similarly as in DERO project
	...
	for i := 0; i < 2*m; i++ {
		anonsupport.f[i][0] = new(big.Int).Mod(new(big.Int).Sub(anonsupport.w, proof.f.vector[i]), bn256.Order)
		anonsupport.f[i][1] = new(big.Int).Set(proof.f.vector[i])
	...
	for i := 0; i < 2*m; i++ {
		anonsupport.temp = new(bn256.G1).Add(anonsupport.temp, new(bn256.G1).ScalarMult(gparams.Gs.vector[i], anonsupport.f[i][1]))

		t := new(big.Int).Mod(new(big.Int).Sub(anonsupport.w, anonsupport.f[i][1]), bn256.Order)
		t = new(big.Int).Mod(new(big.Int).Mul(t, anonsupport.f[i][1]), bn256.Order)

		anonsupport.temp = new(bn256.G1).Add(anonsupport.temp, new(bn256.G1).ScalarMult(gparams.Gs.vector[2*m+i], t))
	}

	t0 := new(bn256.G1).ScalarMult(gparams.Gs.vector[0+4*m], new(big.Int).Mod(new(big.Int).Mul(anonsupport.f[0][1], anonsupport.f[m][1]), bn256.Order))
	t1 := new(bn256.G1).ScalarMult(gparams.Gs.vector[1+4*m], new(big.Int).Mod(new(big.Int).Mul(anonsupport.f[0][0], anonsupport.f[m][0]), bn256.Order))
	...
	if stored.String() != computed.String() { // if failed bail out
		klog.Warning("Recover key failed")
		return false
	}
	...
	C_p := Convolution(NewFieldVector(p), NewGeneratorVector(s.C))
	C_q := Convolution(NewFieldVector(q), NewGeneratorVector(s.C))
	y_p := Convolution(NewFieldVector(p), NewGeneratorVector(s.Publickeylist))
	y_q := Convolution(NewFieldVector(q), NewGeneratorVector(s.Publickeylist))
	...
	protsupport.tEval = new(bn256.G1).ScalarMult(proof.tCommits.vector[0], x)
	protsupport.tEval.Add(new(bn256.G1).Set(protsupport.tEval), new(bn256.G1).ScalarMult(proof.tCommits.vector[1], xsq))
	...
	right := new(bn256.G1).ScalarMult(anonsupport.CR[0][0], zs0_neg)
	...
	u_x := new(bn256.G1).ScalarMult(gparams.G, o)
